...
def send_message(sender, receiver, message, active_nodes, route_table, path = None, heartbeat = False):
        ...
        for key in active_nodes.keys():
            if key == receiver:
                ...
                if path != None:
                    ...
                    if heartbeat != False:
                        message_to_send = bytes("||".join(["2", sender, message, ":".join(path)]), encoding="ascii")
